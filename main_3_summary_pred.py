...
def normalize_label(x):
    """Return a cleaned string for label comparison.

    - Coerce to str (unless NaN -> empty string)
    - Trim surrounding whitespace
    - Remove surrounding single/double quotes if present
    - Collapse multiple inner whitespace to single space
    """
    if pd.isna(x):
        return ""
    s = str(x).strip()
    ...
    if (s.startswith("'") and s.endswith("'")) or (s.startswith('"') and s.endswith('"')):
        s = s[1:-1].strip()
    # remove stray single/double quotes inside
    s = s.replace("\'", "").replace('"', "")
...
if dataset == "DomainNet":
    ...
    # Normalize label-like columns so comparisons use the same dtype/format
    # (trim quotes, whitespace, collapse inner spaces)
    # apply to both predicted and ground truth columns when present
    for col in ['predicted class name', 'ground truth']:
        if col in df_direct.columns:
            df_direct[col] = df_direct[col].apply(normalize_label)
        if col in df.columns:
            df[col] = df[col].apply(normalize_label)

    # Normalize class_list values as well to match CSV-normalized labels
    class_list = [normalize_label(c) for c in class_list]
    ...
    df = pd.read_csv(f'llm_data/{target_type}_{dataset}_target_domain{domain}_{model_name}_{version}_randomseed{random_seed}_summary_pred.csv', index_col=False)[:num_samples]
...
def process_preds(row):
    ...
    """
    return normalize_label(row.get('predicted class name'))

print(df_direct['predicted class name'].apply(type).value_counts())
print(df_direct['predicted class name'].dtype)

print(df['predicted class name'].apply(type).value_counts())
print(df['predicted class name'].values.dtype)

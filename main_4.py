...
if dataset == "DomainNet":
    ...
    if domain == 0 or domain == 2:
        ...
        # df2 = pd.read_csv(f'{dataset}_target_domain{domain}_4o-mini_summary_v10_2.csv')
    ...
    df_direct_ = pd.read_csv(f"target_domain{domain}_4o-mini_v8_1.csv", index_col=False)
    ...
    df = pd.read_csv(f'{dataset}_target_domain{domain}_{model_name}_summary_v10.csv', index_col=False)
    df_direct = pd.read_csv(f"{dataset}_target_domain{domain}_{model_name}_v8.csv", index_col=False)
...
# cos_sim_df.to_csv(f"{dataset}_Domain_{domain} similarity_score_add_context_to_labels.csv", header=False, index=False)
# cos_sim_w_llm_df.to_csv(f"{dataset}_Domain_{domain} similarity_score_add_context_to_labels_w_llm_preds.csv", header=True, index=False)
...
known_ratios = [np.exp(list(similarity_scores_w_llm_preds['scores'].values)[idx] / tau) / np.sum(np.exp(similarity_scores_w_all_lbls[idx]) / tau) for idx in known_in_idx]
unknown_ratios = [np.exp(list(similarity_scores_w_llm_preds['scores'].values)[idx] / tau) / np.sum(np.exp(similarity_scores_w_all_lbls[idx]) / tau) for idx in unknown_in_idx]
ratios = [known_ratios, unknown_ratios]
known_median = np.median(known_ratios)
unknown_median = np.median(unknown_ratios)
fig = plt.figure(figsize=(32, 14))
...
plt.yticks(rotation=0, fontsfontsizeize=20)
...
plt.savefig(f"{dataset}_known_unknown_add_label_context_w_llm_preds_ori.png")

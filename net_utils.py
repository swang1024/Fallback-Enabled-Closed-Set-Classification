...
def compute_aucroc_f1(args, gt_label_all, pred_cls_all, open_flag=True, pred_unc_all=None):
    ...
    gt_label_all_binary = [1 if item == cls_num else 0 for item in gt_label_all]

    auc = roc_auc_score(gt_label_all_binary, pred_unc_all)
    # f1_score = f1_score(gt_label_all_binary, pred_unc_all)

    return auc

def compute_performance(args, class_list, gt_label_all, pred_cls_all, open_flag=True, open_thresh=0.5, pred_unc_all=None):
    # class_list:
    #   :source [0, 1, ..., N_share - 1, ...,           N_share + N_src_private - 1]
    #   :target [0, 1, ..., N_share - 1, N_share + N_src_private + N_tar_private -1]
    # gt_label_all [N]
    ...
    for i, label in enumerate(args.source_class_list):
        label_idx = torch.where(gt_label_all == label)[0]
        correct_idx = torch.where(pred_label_all[label_idx] == label)[0]
        per_class_num[i] = float(len(label_idx))
        per_class_correct[i] = float(len(correct_idx))

    src_per_class_acc = per_class_correct / (per_class_num + 1e-5)
    matrix = confusion_matrix(gt_label_all.cpu(), pred_label_all.cpu(), labels=list(set(args.target_class_list+args.source_class_list)))
    return src_per_class_acc, matrix

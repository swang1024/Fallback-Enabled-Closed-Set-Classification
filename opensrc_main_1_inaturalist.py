...
if model_name == "BLIP2":  
    ...
    for idx, (batch, imgs_train, imgs_label, imgs_idx, ground_truth, private) in enumerate(target_train_dataloader): 
        ...
        df.to_csv(os.path.join("llm_data/{}_{}_target_domain{}_llama_{}_randomseed{}.csv".format(target_type, dataset, args.t_idx, version, random_seed)), index=False)
    ...
    for idx, (imgs_train, imgs_label, imgs_idx, ground_truth, private) in enumerate(target_train_dataloader): 
        ...
        df = pd.DataFrame(data, columns=columns)
        df.to_csv(os.path.join("llm_data/{}_{}_target_domain{}_qwen_{}_randomseed{}.csv".format(target_type, dataset, args.t_idx, version, random_seed)), index=False)

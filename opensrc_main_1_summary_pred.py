...
if model_name == "BLIP2":  
    ...
    if dataset == 'DomainNet':
        ...
        user_promptv8 = "Does this image belong to one of the categories in the following list \
        ['aeroplane', 'bicycle', 'bus', 'car', 'horse', 'knife', 'motorcycle', 'person', 'plant'] based on the follow image summary: {}? \
        Please format the answer csv format with keys unknown and class_name separated by ',' \
        Example 1: \
    ...
    for idx, (imgs_train, imgs_label, imgs_idx, ground_truth, private) in enumerate(target_train_dataloader): 
        ...
        # print(imgs_idx[0].cpu().numpy(), imgs_train)
        messages = [
            {
                "role": "system",
                "content": [
            ...
            },
            {
                "role": "user",
                "content": [
                    {"type": "text", "text": user_promptv8.format(summary)}
                ]
            }
        ]
        ...
        if "unknown: " in answer and ", class_name: " in answer:
            ...
            data.append([imgs_idx.cpu().numpy()[0], list(ground_truth)[0], class_name, private[0], unknown, imgs_train[0]])
        df = pd.DataFrame(data, columns=columns)
        df.to_csv(os.path.join("llm_data/{}_target_domain{}_llama_{}_summary_pred.csv".format(dataset, args.t_idx, version)), index=False)
    ...
    df_summary = pd.read_csv(f'llm_data/{dataset}_target_domain{args.t_idx}_{model_name}_summary_{version}.csv', sep=",", index_col=False) 
    ...
    for idx, (imgs_train, imgs_label, imgs_idx, ground_truth, private) in enumerate(target_train_dataloader): 
        ...
        summary = df_summary[df_summary['idx'] == imgs_idx[0].cpu().numpy()]['summary'].values[0]
        # print(user_promptv8.format(summary))
        messages = [
            {
                "role": "system",
                "content": [
        ...
        df = pd.DataFrame(data, columns=columns)
        df.to_csv(os.path.join("llm_data/{}_target_domain{}_qwen_{}_summary_pred.csv".format(dataset, args.t_idx, version)), index=False)

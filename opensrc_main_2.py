...
if model_name == "BLIP2":  
    ...
    for idx, (batch, imgs_train, imgs_label, imgs_idx, ground_truth, private) in enumerate(target_train_dataloader): 
        ...
        df.to_csv(os.path.join("llm_data/{}_target_domain{}_llama_summary_{}.csv".format(dataset, args.t_idx, version)), index=False)
    ...
    for idx, (imgs_train, imgs_label, imgs_idx, ground_truth, private) in enumerate(target_train_dataloader): 
        ...
        df = pd.DataFrame(data, columns=columns)
        df.to_csv(os.path.join("llm_data/{}_target_domain{}_qwen_summary_{}.csv".format(dataset, args.t_idx, version)), index=False)

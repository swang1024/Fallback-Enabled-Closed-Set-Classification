...
if dataset == "DomainNet":
    ...
    df_direct = pd.read_csv(f"{dataset}_target_domain{domain}_{model_name}_v8.csv", index_col=False)[:12000]
...
if not load_scores:
    ...
    cos_sim_df.to_csv(f"{dataset}_Domain_{domain} similarity_score_add_context_to_labels_{model_name}_{version}.csv", header=False, index=False)
    cos_sim_w_llm_df.to_csv(f"{dataset}_Domain_{domain} similarity_score_add_context_to_labels_w_llm_preds_{model_name}_{version}.csv", header=True, index=False)
    similarity_scores_w_all_lbls = pd.read_csv(f"{dataset}_Domain_{domain} similarity_score_add_context_to_labels_{model_name}_{version}.csv", header=None, index_col=False).to_numpy()
    similarity_scores_w_llm_preds = pd.read_csv(f"{dataset}_Domain_{domain} similarity_score_add_context_to_labels_w_llm_preds_{model_name}_{version}.csv", index_col=False)
...
print(f"{model_name} {version} desc w/ {num_samples} samples")

...
if dataset == "DomainNet":
    ...
    df = pd.read_csv(f'llm_data/{dataset}_target_domain{domain}_qwen_summary_v10_randomseed{random_seed}_300tokens.csv', index_col=False)[:num_samples]
    df_direct = pd.read_csv(f"llm_data/{dataset}_target_domain{domain}_qwen_v8_randomseed{random_seed}.csv", index_col=False)[:num_samples]
...
def plot_tsne(embeddings1, embeddings2, labels=('Set 1', 'Set 2'), title='t-SNE Visualization'):
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(f"{second_opinion_model}_tsne.png")
...
if not load_scores:
    if second_opinion_model == "CLIP":
        ...
        sentence_embedding = []
        for i in range(len(pred_summary[:4000])):
        ...
        for i in range(len(llm_preds_w_context[:4000])):
    ...
    plot_tsne(sentence_embedding[:4000], class_list_w_context_embeddings)

...
# else:
#     cos_sim_df.to_csv(f"Domain {domain} similarity_score_add_context_to_labels.csv", header=False, index=False)
...
if mean_centering:
    similarity_scores = pd.read_csv(f"Domain {domain} similarity_score_add_context_to_labels_mean_centering_independent.csv", header=None, index_col=False).to_numpy()
else:
    similarity_scores = pd.read_csv(f"Domain {domain} similarity_score_add_context_to_labels.csv", header=None, index_col=False).to_numpy()
...
# for known with right pred
known_correct_idx = [idx for idx in known_idx if preds_all[idx] == gt[idx]]
known_correct_ratios = np.exp(np.max(similarity_scores[known_correct_idx], axis=1)) / np.sum(np.exp(similarity_scores[known_correct_idx]), axis=1)

# for known with incorrect pred
known_incorrect_idx = [idx for idx in known_idx if preds_all[idx] != gt[idx]]
known_incorrect_ratios = np.exp(np.max(similarity_scores[known_incorrect_idx], axis=1)) / np.sum(np.exp(similarity_scores[known_incorrect_idx]), axis=1)

preds_known = preds_all[known_idx]
# most_sim_lbl_known = [most_similar_label_w_context[pred] for pred in preds_known]
known_ratios = [np.exp(similarity_scores[idx, i]) / np.sum(np.exp(similarity_scores[idx])) for idx, i in zip(known_idx, preds_known)]
# known_ratios_small_scale = [np.exp(similarity_scores[idx, i]) / (np.exp(similarity_scores[idx, i]) + np.exp(similarity_scores[idx, j])) for idx, i, j in zip(known_idx, preds_known, most_sim_lbl_known)]
...
unknown_ratios = [np.exp(similarity_scores[idx, i]) / np.sum(np.exp(similarity_scores[idx])) for idx, i in zip(unknown_idx, preds_unknown)]
# unknown_ratios_small_scale = [np.exp(similarity_scores[idx, i]) / (np.exp(similarity_scores[idx, i]) + np.exp(similarity_scores[idx, j])) for idx, i, j in zip(unknown_idx, preds_unknown, most_sim_lbl_unknown)]
...
plt.legend(['unknown', 'known_incorrect', 'known_correct'], fontsize=30)
...
if mean_centering:
    ...
    plt.savefig("known_corrrect_incorrect_unknown_add_label_context.png")
...
# # plt.legend()
# # plt.savefig("direct_vs_indirect_labeling.png")
